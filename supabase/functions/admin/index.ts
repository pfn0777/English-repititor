...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { action, username, password, settings, userId, limit, q, ent, days } = await req.json();
    ...
    // Qo'lda obuna berish / bekor qilish.
    if (action === 'grant_sub') {
      ...
      const { data: u, error: readErr } = await db.from('users')
        .select('id,tg_id,tg_username,trial_started_at,subscription_until')
        .eq('id', userId).maybeSingle();
      if (readErr) {
        console.error('grant_sub_read', readErr.message);
        return json({ error: 'read_failed', message: "Foydalanuvchi o'qilmadi" }, 500, cors);
      }
      if (!u) return json({ error: 'not_found', message: 'Foydalanuvchi topilmadi' }, 404, cors);
      ...
      if (d === -1) {
        const { error } = await db.from('users').update({ subscription_until: null }).eq('id', userId);
        if (error) {
          console.error('grant_sub_cancel', error.message);
          return json({ error: 'update_failed', message: 'Bekor qilinmadi' }, 500, cors);
        }
        const next = { trial_started_at: u.trial_started_at, subscription_until: null };
        return json({ ok: true, subscription_until: null, ent: entitlementOf(next, now), days_left: daysLeft(next, now) }, 200, cors);
      }
      ...
      const { error: updErr } = await db.from('users').update({ subscription_until: until }).eq('id', userId);
      if (updErr) {
      ...
      const { error: payErr } = await db.from('payments').insert({
        ...
        user_id: u.id,
        stars: 0,
        charge_id: `manual:${u.id}:${new Date(now).toISOString()}`,
        payload: `admin:+${d}d`,
        source: 'manual',
      });
      if (payErr) console.error('grant_sub_audit', payErr.message);

      const next = { trial_started_at: u.trial_started_at, subscription_until: until };
      return json({
        ok: true,
